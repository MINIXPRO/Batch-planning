...
frappe.ui.form.on("Material Allocation", {
    ...
    refresh: function (frm) {
        ...
        if (frm.doc.allocation_status) {
            ...
            frm.set_df_property("material_allocation", "editable_grid", 0);
            frm.refresh_field("material_allocation");
        } else {
            frm.fields_dict["material_allocation"].grid.editable_grid = true;
            frm.fields_dict["material_allocation"].grid.update_docfield_property("item_code", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("item_name", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("uom", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("quantity_required", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("stock_available", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("open_pr", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("open_po", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("grn_qty", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("qty_allocated", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("shortage", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("batch_details", "read_only", 1);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("allocate_qty", "read_only", 0);
            frm.fields_dict["material_allocation"].grid.update_docfield_property("reason", "read_only", 0);
            frm.refresh_field("material_allocation");

            // Force row-level read-only for item_code, quantity_required, and stock_available
            let grid = frm.fields_dict["material_allocation"].grid;
            if (grid && grid.grid_rows) {
                grid.grid_rows.forEach(row => {
                    row.toggle_editable("item_code", false);
                    row.toggle_editable("quantity_required", false);
                    row.toggle_editable("stock_available", false);
                });
            }
        }
        ...
        // ── View Allocation History Button ──
    ...
    validate: function (frm) {
        ...
        if (bom_error) {
            frappe.msgprint({ title: __("Validation Failed"), message: __(bom_error), indicator: "red" });
            frappe.validated = false;
            return;
        }

        if (reason_error) {
            frappe.msgprint({ title: __("Validation Failed"), message: __(reason_error), indicator: "red" });
            frappe.validated = false;
        }
    },
...
window._show_item_history = function (item_code) {
    ...
    if (!filtered.length) {
        frappe.msgprint({ title: "No Events", message: `No events found for ${item_code}`, indicator: "orange" });
        return;
    }
    ...
    let ed = new frappe.ui.Dialog({ title: `📦 Events for: ${item_code}`, size: "extra-large" });
    ed.body.innerHTML = `
        <table class="table table-bordered" style="width:100%;font-size:13px;">
            <thead style="background:#f1f5f9;">
                <tr>
            ...
            frappe.confirm(
                ...
                function () {
                    frm.call("deallocate").then((r) => {
                        if (!r.exc) {
                            frappe.show_alert({ message: __("✅ Deallocated successfully!"), indicator: "blue" });
                            frm.reload_doc();
                        }
                    });
                },
...
window.upload_bom_items = function (frm) {
    ...
    frappe.call({
        ...
        callback: function (r) {
            if (!r.message || !r.message.length) {
                frappe.msgprint({ title: "No Items", message: "No BOM items found.", indicator: "orange" });
                return;
            }
