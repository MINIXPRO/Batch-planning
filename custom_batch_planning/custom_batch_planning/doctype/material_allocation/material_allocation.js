...
//         let is_draft = frm.doc.workflow_state === "Draft";
//         frm.set_df_property("material_allocation", "cannot_add_rows", 1);
//         frm.set_df_property("material_allocation", "cannot_delete_rows", is_draft ? 0 : 1);
...
frappe.ui.form.on("Material Allocation", {
    ...
    refresh: function (frm) {
        ...
        // ── Grid: no add rows; allow delete but enforce min 1 row ──
        frm.set_df_property("material_allocation", "cannot_add_rows", true);
        ...
        // ── Allocation History Button (visible on any saved doc with batch_planning) ──
        if (!frm.is_new() && frm.doc.batch_planning) {
            frm.add_custom_button(__("📋 Allocation History"), function () {
                frappe.call({
                    method: "frappe.client.get_list",
